...
The GCC options -no-pie, -z execstack, and -fno-stack-protector are all disabling defensive mechanisms. -z execstack disables W xor X with respect to the stack, making the stack executable as well as writable. -no-pie, disables positional execution, and -fno-stack-protector turns off the stack protector. If you look at the source code for this program you'll see that the other simple thing about it is the vulnerability. The function read_and_print has a fixed-size buffer ri.buf, and it copies the entire contents of the file specified on the command line into that buffer with the read system call, with only an insufficient check on the length. However, the length check is enough to prevent overwrite the function's return address. So there's a different piece of control data you'll need to overwrite instead.<br>
